...
int8 page_refcnt_increase(uint64 pa) {
    ...
    /* pa – RISCV_DDR_BASE 得到与物理基址的距离，再除以 PGSIZE 得到页号 */
    idx = (pa - RISCV_DDR_BASE) >> PGSHIFT;
...
int mm_mappages_cow(struct vma *vma, struct vma *oldvma) {
    ...
    for (va = vma->vm_start; va < vma->vm_end; va += PGSIZE) {
        ...
        /* 2. 引用计数 +1 */
        page_refcnt_increase(pa);

        /* 3. 给子进程写一条去掉 PTE_W、新增 PTE_COW 的映射 */
        uint64 perm = (*ppte & (PTE_R | PTE_X | PTE_U)) | PTE_A3_COW;
        if (mm_mappageat(mm, va, pa, perm) < 0) {
            page_refcnt_decrease(pa);
            ret = -ENOMEM;
            goto bad;
        }

        /* 4. 如果父页有写权限，也改为只读 + COW */
        if (*ppte & PTE_W) {
            *ppte = (*ppte & ~PTE_W) | PTE_A3_COW;
        }
    }
    sfence_vma();
...
bad:
    /* 回滚已经映射/计数的页 */
    for (uint64 rva = vma->vm_start; rva < va; rva += PGSIZE) {
        pte_t *cpte = walk(mm, rva, 0);
        if (cpte && (*cpte & PTE_V)) {
            uint64 cpa = PTE2PA(*cpte);
            page_refcnt_decrease(cpa);
            *cpte = 0;
        }
    }
    freevma(vma, true);
